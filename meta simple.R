...
covar <- effect$Effectsize
#change covar to whatever covar you want, and then rerun the code
metareg <- metareg(metacont,covar,intercept=FALSE)
metareg
#Not sure if metareg is using the fixed effects or random effects output of the metacont function
...
model2 <- rma(yi = metacont$TE, 
              ...
              data = metacont, 
              method = "ML", 
              mods = ~ effect$Change.Fx + effect$severity, 
              test="knha")
model2
...
interaction.model <- rma(yi=metacont$TE,
                         ...
                         data=metacont, 
                         method = "REML", 
                         mods = ~ effect$Effectsize*effect$Change.Fx, 
                         test="knha")
interaction.model
